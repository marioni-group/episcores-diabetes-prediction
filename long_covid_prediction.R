...
lcTable <- read.csv('/path/to/file')
lcTable <- lcTable[!is.na(lcTable$S3_SymptomLengthAll), ]
lcTable <- lcTable[lcTable$S3_SymptomLengthAll > 2, ]
...
targetW1$long_covid <- sapply(targetW1$Sample_Name, function(id) {ifelse(id %in% lcTable$id, 1, 0)})
...
targetW1$closestTestDate <- sapply(1:nrow(targetW1), function(i) {
  ...
  if (id %in% lcTable$id) {
    lcDecimal <- lcTable[lcTable$id == id, 'dateDecimal']
  } else {
    lcDecimal <- NULL
  }
  ...
  # print(dateDecimals)
  if (!is.null(lcDecimal)) {
    distances <- sapply(dateDecimals, function(d) {abs(d - lcDecimal)})
    # print(distances)
...
nullModel <- glm(long_covid ~ Age + sex + bmi + family_diabetes + high_BP + Event, family = 'binomial', data = targetW1)
pModel <- glm(long_covid ~ Age + sex + bmi + family_diabetes + high_BP + Event + P, family = 'binomial', data = targetW1)
dModel <- glm(long_covid ~ Age + sex + bmi + family_diabetes + high_BP + Event + D, family = 'binomial', data = targetW1)
fullModel <- glm(long_covid ~ Age + sex + bmi + family_diabetes + high_BP + Event + P + D, family = 'binomial', data = targetW1)
...
targetW1Cases <- targetW1[targetW1$long_covid == 1, ]
targetW1Controls <- targetW1[targetW1$long_covid == 0, ]
