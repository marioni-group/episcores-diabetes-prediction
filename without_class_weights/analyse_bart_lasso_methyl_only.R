...
source('/Cluster_Filespace/Marioni_Group/Yipeng/prediction-pipelines/incident_IHD_pipeline/src/pipeline_modules.R')

runInformation <- beginPipelineRun(note = 'Analysis of BART survival model from 2021_03_29_12_12_56 with features selected with logistic lasso - no class weights, methylation only. NA covariate rows removed. Train on wave 3, test on wave 1. Test data is filtered by TTE threshold. Deaths removed from controls. event.predictions object saved.', logFolderPath = '/Cluster_Filespace/Marioni_Group/Yipeng/prediction-pipelines/incident_diabetes_pipeline/using_methylpiper/logs/final_pipeline_instances/', log = TRUE)
...
testData <- loadData('/Cluster_Filespace/Marioni_Group/Yipeng/prediction-pipelines/incident_diabetes_pipeline/data/training_data.rds')
testTarget <- loadData('/Cluster_Filespace/Marioni_Group/Yipeng/prediction-pipelines/incident_diabetes_pipeline/data/training_target.csv')
...
bartSurvivalModel <- readRDS(paste0(runInformation[['log folder path']], 'bart_survival_model_2021_03_29_12_12_56.rds'))
...
if (runInformation[['log']]) {
  saveRDS(eventPredictions, paste0(runInformation[['log folder path']], 'bart_survival_model_from_2021_03_29_12_12_56_event_predictions_', runInformation[['start timestamp']], '.rds'))
}
