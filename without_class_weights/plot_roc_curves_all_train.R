library(MethylPipeR)
runInformation <- beginPipelineRun(note = 'Plot train ROC curves. All models', log = FALSE,
...
lassoResponse <- readRDS(paste0(runInformation[['log folder path']], 'train_response_logistic_model_predictions_2021_03_16_14_34_18.rds'))
elnetResponse <- readRDS(paste0(runInformation[['log folder path']], 'train_response_logistic_model_predictions_2021_03_16_14_17_33.rds'))
ridgeResponse <- readRDS(paste0(runInformation[['log folder path']], 'train_response_logistic_model_predictions_2021_03_16_14_46_51.rds'))
lassoNullResponse <- readRDS(paste0(runInformation[['log folder path']], 'train_response_logistic_model_predictions_2021_03_16_16_55_07.rds'))
srfResponse <- readRDS(paste0(runInformation[['log folder path']], 'survival_random_forest_train_response_2021_03_18_15_33_15.rds'))
coxElnetResponse <- readRDS(paste0(runInformation[['log folder path']], 'cox_elnet_onset_predictions_train_2021_03_18_09_10_52.rds'))

methods <- c('survival_random_forest', 'logistic_ridge', 'logistic_elnet', 'logistic_lasso', 'cox_elnet', 'logistic_lasso_null')
responses <- list(srfResponse, ridgeResponse, elnetResponse, lassoResponse, coxElnetResponse, lassoNullResponse)
...
calibrationResult <- caret::calibration(actual ~ logistic_lasso + logistic_elnet + logistic_ridge + logistic_lasso_null + survival_random_forest + cox_elnet,
                                        data = calibrationDF)
