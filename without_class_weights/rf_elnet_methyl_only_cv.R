...
runInformation <- beginPipelineRun(note = 'Random forest with elnet-selected variables, methylation only (Experiment timestamp: 2021_03_31_10_45_31. No class weights). Train on wave 3, test on wave 1.', 
                                   logFolderPath = '/path/to/log/folder/',
                                   log = TRUE)

selectedFeatures <- read.csv(paste0(runInformation[['log folder path']], 'logistic_elnet_model_non_zero_model_coefficients_2021_03_31_10_45_31.csv'))[-1, 'X']
...
if (runInformation[['log']]) {
  saveRDS(rfTrainResponse, paste0(runInformation[['log folder path']], 'random_forest_elnet_train_response_', runInformation[['start timestamp']], '.rds'))
  saveRDS(rfTestResponse, paste0(runInformation[['log folder path']], 'random_forest_elnet_test_response_', runInformation[['start timestamp']], '.rds'))
}
