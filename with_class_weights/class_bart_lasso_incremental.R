...
runInformation <- beginPipelineRun(note = 'Probit BART with lasso features 10-year survival incremental model. Probit BART timestamp: 2021_05_04_12_37_44. Variables: CpGs, age, sex, bmi, self-reported parent/sibling diabetes, self-reported hypertension. NA covariate rows removed. With class weights', 
...
bartResponse <- readRDS(paste0(runInformation[['log folder path']], 'class_bart_test_response_2021_05_04_12_37_44.rds'))
...
if (runInformation[['log']]) {
  saveRDS(nullModel, paste0(runInformation[['log folder path']], 'incremental_class_bart_lasso_null_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModel, paste0(runInformation[['log folder path']], 'incremental_class_bart_lasso_full_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(nullModelResponse, paste0(runInformation[['log folder path']], 'incremental_class_bart_lasso_null_model_response_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModelResponse, paste0(runInformation[['log folder path']], 'incremental_class_bart_lasso_full_model_response_', runInformation[['start timestamp']], '.rds'))
}
