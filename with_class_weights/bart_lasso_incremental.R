...
runInformation <- beginPipelineRun(note = 'BART with lasso features 10-year survival incremental model. Variables: CpGs, age, sex, bmi, self-reported parent/sibling diabetes, self-reported hypertension. NA covariate rows removed. With class weights', 
...
bartResponse <- readRDS(paste0(runInformation[['log folder path']], 'bart_survival_model_from_2021_04_25_14_24_45_event_predictions_2021_04_26_11_37_10.rds'))
...
if (runInformation[['log']]) {
  saveRDS(nullModel, paste0(runInformation[['log folder path']], 'incremental_bart_lasso_null_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModel, paste0(runInformation[['log folder path']], 'incremental_bart_lasso_full_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(nullModelResponse, paste0(runInformation[['log folder path']], 'incremental_bart_lasso_null_model_response_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModelResponse, paste0(runInformation[['log folder path']], 'incremental_bart_lasso_full_model_response_', runInformation[['start timestamp']], '.rds'))
}
