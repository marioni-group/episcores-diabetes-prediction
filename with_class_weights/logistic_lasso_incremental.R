...
runInformation <- beginPipelineRun(note = 'Logistic lasso 10-year survival incremental model. Variables: CpGs, age, sex, bmi, self-reported parent/sibling diabetes, self-reported hypertension. NA covariate rows removed. With class weights. Fit methyl model on wave 3.', 
...
print('nrows in Set 2 before removing NAs')
print(nrow(trainingTarget))
print('ncase in Set 2 before removing NAs')
print(nrow(trainingTarget[trainingTarget$Event == 1,]))
...
print('nrows in Set 2 after removing NAs')
print(nrow(trainingTarget))
print('ncase in Set 2 after removing NAs')
print(nrow(trainingTarget[trainingTarget$Event == 1, ]))
...
trainingThresholdTTEResult <- thresholdTTEResult
...
methylModel <- fitLogisticModel(trainingData,
                                trainingTarget[, 'Event', drop = FALSE],
                                'lasso',
                                trainingWeights,
                                3,
...
methylModelTrainPredictions <- predictLogisticModel(methylModel,
                                                    trainingData,
                                                    runInformation,
                                                    'train_class')

methylModelTrainResponse <- predictLogisticModel(methylModel,
                                                 trainingData,
                                                 runInformation,
                                                 'train_response',
                                                 'response')

methylModelTrainMetrics <- calculateMetricsBinaryClassifier(methylModelTrainPredictions,
                                                            trainingTarget,
                                                            runInformation,
                                                            'train')
...
print('nrows in Set 1 before removing NAs')
print(nrow(testTarget))
print('ncase in Set 1 before removing NAs')
print(nrow(testTarget[testTarget$Event == 1, ]))
...
print('nrows in Set 1 after removing NAs')
print(nrow(testTarget))
print('ncase in Set 1 after removing NAs')
print(nrow(testTarget[testTarget$Event == 1, ]))
...
testThresholdTTEResult <- thresholdTTEResult
thresholdTTEResult <- NULL
gc()
...
methylModelTestPredictions <- predictLogisticModel(methylModel,
                                                   testData,
                                                   runInformation,
                                                   'test_class')

methylModelTestResponse <- predictLogisticModel(methylModel,
                                                testData,
                                                runInformation,
                                                'test_response',
                                                'response')

methylModelTestLink <- predictLogisticModel(methylModel,
                                            testData,
                                            runInformation,
                                            'test_link',
                                            'link')

methylModelTestMetrics <- calculateMetricsBinaryClassifier(methylModelTestPredictions,
                                                           testTarget,
                                                           runInformation,
                                                           'test')

nonZeroCoefficients <- getNonZeroCoefficients(model = methylModel,
                                              pipelineRunInformation = runInformation,
                                              modelLabel = 'logistic_lasso')

testTarget$methyl <- methylModelTestLink
...
if (runInformation[['log']]) {
  saveRDS(nullModel, paste0(runInformation[['log folder path']], 'incremental_logistic_lasso_null_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModel, paste0(runInformation[['log folder path']], 'incremental_logistic_lasso_full_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(nullModelResponse, paste0(runInformation[['log folder path']], 'incremental_logistic_lasso_null_model_response_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModelResponse, paste0(runInformation[['log folder path']], 'incremental_logistic_lasso_full_model_response_', runInformation[['start timestamp']], '.rds'))
}
