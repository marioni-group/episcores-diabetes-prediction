...
runInformation <- beginPipelineRun(note = 'BART with elnet features 10-year survival incremental model. Variables: CpGs, age, sex, bmi, self-reported parent/sibling diabetes, self-reported hypertension. NA covariate rows removed. With class weights', 
...
bartResponse <- readRDS(paste0(runInformation[['log folder path']], 'bart_survival_model_from_2021_05_04_10_13_42_event_predictions_2021_05_04_13_48_04.rds'))
...
if (runInformation[['log']]) {
  saveRDS(nullModel, paste0(runInformation[['log folder path']], 'incremental_bart_elnet_null_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModel, paste0(runInformation[['log folder path']], 'incremental_bart_elnet_full_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(nullModelResponse, paste0(runInformation[['log folder path']], 'incremental_bart_elnet_null_model_response_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModelResponse, paste0(runInformation[['log folder path']], 'incremental_bart_elnet_full_model_response_', runInformation[['start timestamp']], '.rds'))
}
