...
runInformation <- beginPipelineRun(note = 'Survival Random Forest CV with elnet features 10-year survival incremental model. Survival Random Forest timestamp: 2021_05_05_00_45_59. Variables: CpGs, age, sex, bmi, self-reported parent/sibling diabetes, self-reported hypertension. NA covariate rows removed. With class weights', 
...
srfResponse <- readRDS(paste0(runInformation[['log folder path']], 'survival_random_forest_test_response_2021_05_05_00_45_59.rds'))
...
if (runInformation[['log']]) {
  saveRDS(nullModel, paste0(runInformation[['log folder path']], 'incremental_srf_elnet_cv_null_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModel, paste0(runInformation[['log folder path']], 'incremental_srf_elnet_cv_full_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(nullModelResponse, paste0(runInformation[['log folder path']], 'incremental_srf_elnet_cv_null_model_response_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModelResponse, paste0(runInformation[['log folder path']], 'incremental_srf_elnet_cv_full_model_response_', runInformation[['start timestamp']], '.rds'))
}
