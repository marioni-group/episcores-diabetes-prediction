...
runInformation <- beginPipelineRun(note = 'Random Forest CV with elnet features 10-year survival incremental model. Random Forest timestamp: 2021_05_04_14_33_20. Variables: CpGs, age, sex, bmi, self-reported parent/sibling diabetes, self-reported hypertension. NA covariate rows removed. With class weights', 
...
rfResponse <- readRDS(paste0(runInformation[['log folder path']], 'random_forest_elnet_test_response_2021_05_04_14_33_20.rds'))
...
if (runInformation[['log']]) {
  saveRDS(nullModel, paste0(runInformation[['log folder path']], 'incremental_rf_elnet_cv_null_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModel, paste0(runInformation[['log folder path']], 'incremental_rf_elnet_cv_full_model_', runInformation[['start timestamp']], '.rds'))
  saveRDS(nullModelResponse, paste0(runInformation[['log folder path']], 'incremental_rf_elnet_cv_null_model_response_', runInformation[['start timestamp']], '.rds'))
  saveRDS(fullModelResponse, paste0(runInformation[['log folder path']], 'incremental_rf_elnet_cv_full_model_response_', runInformation[['start timestamp']], '.rds'))
}
