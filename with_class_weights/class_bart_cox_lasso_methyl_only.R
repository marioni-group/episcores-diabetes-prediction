...
runInformation <- beginPipelineRun(note = 'Probit BART (binary classification) with features selected with cox lasso methylation only with class weights (experiment timestamp: 2021_05_04_10_44_41).', 
                                   logFolderPath = '/path/to/log/folder/', 
                                   log = TRUE)

# Here we select rows -1 to exclude the intercept.
selectedFeatures <- read.csv(paste0(runInformation[['log folder path']], 'cox_lasso_model_non_zero_model_coefficients_2021_05_04_10_44_41.csv'))[-1, 'X']
